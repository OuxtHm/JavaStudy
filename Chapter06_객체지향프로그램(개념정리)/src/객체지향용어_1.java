/*
	...
	1. 객체지향 프로그램
		...
		2) 특징(***********)면접
			-> 캡슐화 : 변수 관련 -> 인스턴스 / 정적변수 
				- 데이터를 보호할 목적
				- 경계선
				- 메소드와 변수를 한 개로 묶어서 외부에서 직접 접근하지 못하게 만든다
					-> getter / setter
					    읽기      쓰기 -> lombok
				- 접근 지정어 사용법
				- 웹에서 주로 캡슐화 사용
	...
	--------------
		-> 객체 생명주기
	class -> 설계 -> 메모리에 저장 -> 필요시 사용 -> 메모리 소멸
								 객체.메소드     객체 = null
		         클래스명 객체;	     객체. 변수
				 객체 = new 클래스명();
				 클래스 명 객체 = new 클래스명()
	...
	2)인스턴스 변수
		선언위치 : 클래스 내부 블록 -> 클래스 위나 아래에 선언이 가능
		객체 생성시에 메모리에 저장 -> new를 사용할 때 메모리에 저장(new 사용 시 새로운 메모리가 생성)	
		초기값은 지정하지 않는 경우 -> 자동으로 기본값이 설정된다.
		-> null은 주소를 가지고 있지 않은 경우
		-> 실제로 클래스의 멤버 변수
		-> 클래스 전체 사용 / 다른 클래스에서 사용이 가능
	3)정적변수
		모든 객체가 공유하는 변수
		*** 구분(따로 저장되는 변수, 한 개를 가지고 사용)
		*** 벤치마킹
		컴파일 시 자동 생성
		프로그램 종료 시까지 메모리 유지
		---------------------
	핵심)
		...
		*** 초기화 방법
		...
		----------
			...
			생성자    -> 5
		----------------보통 사용
		인스턴스 변수 ; 반드시 생성자 호출 -> 생성자에서 초기화
		정적 변수 : 자동 저장 -> static{} 이용
		*** 변수는 초기값이 있어야 메모리에 저장이 가능 -> 프로그램에서 사용
	
	생성자
		-> 객체가 생성될 때 호출되는 메소드 형태의 코드 블록
							   
		-> 역할 : 인스턴스 변수에 대한 초기값 대입(초기화 함수)
		-> 시작과 동시에 처리해야되는 기능 설정
			** 자동 로그인 / 아이디 저장
			** 쿠키를 읽어온다 -> 장바구니, 최근 방문
			** 화면 UI -> 레이아웃
		-> 특징
			...
			|_ 변수에 대한 초기값을 변경 -> 매개변수\
			** 모든 클래스는 생성자가 한 개 이상이 반드시 필요하다
				-> 생성자가 없는 경우는 컴파일러가 자동으로 기본 생성자 대입
				-> 기본 생성자 -> 매개변수가 없는 생성자
		-> 생성자의 종류
			기본 생성자(디폴트 생성자)
				-> 시작과 동시에 처리해야 되는 기능 설정
			매개변수가 있는 생성자
				-> 객체마다 다른 데이터를 저장하고싶은 경우
		-> 변수
			기본형 -> int a, double b...
			------------------------기본형(실제값)
			배열 : 같은 데이터형       __
			클래스 : 다른 관련된 데이터형__|___ 참조형 변수(주소)
