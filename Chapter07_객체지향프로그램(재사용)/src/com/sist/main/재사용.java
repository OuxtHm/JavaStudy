...
/*
	재사용법
		...
		1) 상속 
			...
			-> 자바는 단일 상속만 가능
					-------- 다중 상속 가능 : 인터페이스
			-> 상속 내린 클래스 : 부모 클래스, 슈퍼 클래스, 베이스 클래스, 상위 클래스
...
public class 재사용 {
	...
	public static void main(String[] args) {
		...
		*/
		...
		s.age = 30;
		s.name = "홍길동";
		System.out.println("나이: " + s.age + "   이름: " + s.name);
		s.eat();
		s.run();
		...
		sub.age = 20;
		sub.name = "심청이";
		System.out.println("나이: " + sub.age + "   이름: " + sub.name);
		sub.eat();
		sub.run();
		...
		System.out.println("나이: " + ss.age + "   이름: " + ss.name);
		ss.eat();
		ss.run();
