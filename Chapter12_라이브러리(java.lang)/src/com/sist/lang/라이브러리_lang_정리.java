...
/*
	...
	1) Object : 모든 클래스의 상속을 내리는 클래스(최상위 클래스)
			...
			주요기능
			 ...
			 void finalized() : 소멸자
			 int hashCode() : 객체의 메모리 주소를 기반으로 정수형으로 만든 구분
	2) String : 문자열을 저장하는 데이터형
		사용법) *****String s = ""; -> 일반 데이터형과 동일
			   String s = new String(""); -> 클래스 형태
		주요 기능
			문자의 갯수 읽기 : length() -> int length()
			문자 그대로 가지고 오기 : toString(); -> String toString();
			문자 변환 : replace(), replaceAll()
					-> String replace(char c1, char c2)
					   ...
					   String replaceAll(String regex, String s);
					   					--------------
					   					정규식
			문자 찾기 : int indexOf(char c), int indexOf(String s)
					  int lastIndexOf(char c), int startIndexOf(char c)
			문자 자르기 : substring(), split() 
						--------
						String substring(int b);
						String substring(int b1, int e);
						String[] split(String regex);
										----------- 정규식을 이용
						-> 보완 : StringTokenizer
			공백문자 제거 : String trim() - 좌우 공백 제거
	...
	6) Wrapper : 기본 데이터형을 쉽게 제어가 가능하게 만든 클래스
					- 제네릭을 이용하는 목적 데이터형을 통일할 경우에 사용 <클래스>
					- 문자열을 해당 데이터형으로 변경
	...
	7) MessageFormat : SQL문장 / 복자한 문장의 출력 형식
	8) DecimalFormat : 숫자표현 세자리 앞에 ....
						##, ### -> 오라클 : 99,999
						DecimalFormat d = new DecimalFormat("#,###,###");
	9) SimpleDeteFormat : 날짜 변환
					년도 : yyyy / yy
					월  :  MM / M
						 1~9
					일  :  dd / d  	
					...
					분 : mm / m
					초 : ss / s
						SimpleDeteFormat sdf = new SimpleDeteFormat("yyyy-MM-dd");
