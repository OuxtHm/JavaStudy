...
/*
	java.lang
	 ...
	 -> import를 생략 간으
		1. Object
			-> 모든 클래스의 상위 클래스(사용자 정의, 라이브러리 클래스 -> 상속 을 내리는 클래스)
			-> 데이터형 중 가장 큰 데이터형
			-> 모든 데이터형을 받을 수 있음
		...
		----------------------------------------
			클래스 객체 = new 클래스()
			----			----
			  |_______________| 다른 경우도 있다
			  	객체가 접근 --> 선언된 클래스가 가지고 있는 변수, 메소드
			  	A a = new B()
			  	---
			  	 | 오버라이딩 된 경우에만 B가 가지고 있는 메소드에 접근
			  	 class A
			  	 {
			  	 	int a, b;
			  	 	public void aaa(){}
			  	 ...
			  	 A a = new B();
			  	 --
			  	 접근 가능한 것 a, b, aaa(), bbb();
			  	 				  ------ B가 가지고 있는 메소드 호출(오버라이딩이 되었기 때문)
			  	 주요 메소드(기능) -> 오버라이딩
			  	 - finalized() : 객체 소멸 시 자동 호출
			  	 	-> 소멸자 함수 : 재정의 하지 않는다.
			  	 - ***toString() : 객체를 문자열 변경
			  	 	-> 오버라이딩이 가장 많다
			  	 	...
			  	 	-> 오버라이딩 -> 실제 객체의 변수값 출력
			  	 - ***clone() : 복제 
			  	 			  실제 가지고 있는 변수 값을 포함하여 새로운 메모리에 저장
			  	 			  -> 스프링 : prototype
			  	 			  -------- singleton
			  	 - equals() -> 많이 사용하지 않는다
			  	 			-> 객체의 주소 값 비교 -> String에서는 문자열만 비교
...
public class 라이브러리_5 {
	...
	public static void main(String[] args) {
		...
		// equals를 재정의하지 않는 경우 -> 메모리 주소 값 비교
		if(h1.equals(h2))
		{
